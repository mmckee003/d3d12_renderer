...
// Key codes are based off of Win32 key codes so no translations needs to 
// be down between the platform and application layer.
enum class Key : u32
{
    ...
    // For some reason EXECUTE causes a compilation error???
    // Is it a macro defined somewhere??
	DEFINE_KEY(EXECUTEE, 0x2B),
